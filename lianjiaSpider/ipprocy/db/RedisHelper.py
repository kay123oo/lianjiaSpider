...
from lianjiaSpider.ipprocy import config
from lianjiaSpider.ipprocy.db.ISqlHelper import ISqlHelper
from lianjiaSpider.ipprocy.db.SqlHelper import Proxy
...
class RedisHelper(ISqlHelper):
    ...
    def selectIps(self, count=None, conditions=None):
        count = (count and int(count)) or 1000  # 最多返回1000条数据
        count = 1000 if count > 1000 else count
        querys = {k: v for k, v in conditions.items() if k in self.index_names} if conditions else None
        if querys:
        ...
        for name in objects:
            p = self.get_proxy_by_name(name)
            result.append((p.ip, p.port, p.protocol))
        return result
