...
class GetQuerying():
    ...
    def getQ_by_rent_type(self, rentTypes):
        resQ = Q()
        for rentType in rentTypes:
            print(rentType)
            if int(rentType) == RentTypeConditions.Entire_Rent.value:
                resQ = Q(dataDistributionType=0)
            elif int(rentType) == RentTypeConditions.Joint_Rent.value:
    ...
    def getQ_by_rentType(self, rentTpye):
        ...
        if rentTpye == RentTypeConditions.Joint_Rent.value:
            resQ = Q(title__contains='合租') & Q(dataDistributionType=0)
        elif rentTpye == RentTypeConditions.Entire_Rent.value:
            resQ = Q(title__contains='整租') & Q(dataDistributionType=0)
        elif rentTpye == RentTypeConditions.Apartment.value:
            resQ = Q(dataDistributionType=1)
        return resQ
